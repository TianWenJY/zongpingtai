<template>
  <div style="margin-top: 20px;">
    ...
    <el-row class="containt">
      ...
      <!--表格内容栏-->
      <!-- @findPage="findPage" -->
      <kt-table :highlightCurrentRow="true" :stripe="false" @findPage="findPage" :data="pageResult" :pageRequest="pageRequest" :columns="columns" :showBatchDelete="false" @handleCurrentChange="handleRoleSelectChange" @handleEdit="handleEdit" @handleDelete="handleDelete">
      </kt-table>
      <!-- </el-col> -->
      <!--新增编辑界面-->
...
export default {
  ...
  methods: {
    ...
    findPage: function(data) {
      ...
      this.findTreeData({ plate_id: this.plate_id });
    },
    // 删除
    handleDelete(row) {
      this.Delete(
    ...
    // 获取数据
    findTreeData: function(plate_id) {
      this.menuLoading = true;
      this.post("PubInterface/Menu/QueryMenuList", plate_id, res => {
        if (res.body.Status == 0) {
          if (this.plate_id == -1) {
            this.menuData = [
              { name: "总平台", id: 1, Children: res.body.Data[1][0] },
              { name: "汽车服务", id: 2, Children: res.body.Data[2][0] }
            ];
          } else {
